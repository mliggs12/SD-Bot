...
export function extractCallingNumber(): CallingNumberResult {
  try {
    ...
    const callingNumberElements = Array.from(document.querySelectorAll(RINGCENTRAL_SELECTORS.callingNumberBold))
    ...
    return { success: false, error: 'Calling number not found' };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return { success: false, error: `Error extracting calling number: ${errorMessage}` };
  }
}
