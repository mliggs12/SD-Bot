...
async function main() {
  ...
  try {
    ...
    if (!maxTab.id) {
      resultDiv.innerHTML = '<div class="error">X MAX tab ID not found</div>';
      resultDiv.className = 'error';
      return;
    }
    ...
    if (callingNumber && callingNumber !== 'Calling number not found') {
      ...
      if (!searchTab.id) {
        resultDiv.innerHTML = '<div class="error">X Search tab ID not found</div>';
        resultDiv.className = 'error';
        return;
      }
      ...
      if (requesterData && requesterData.found && requesterData.scenario === 1) {
        ...
        // Update the sidepanel display
        const requesterNameSpan = document.getElementById('requester-name');
        const phoneNumberSpan = document.getElementById('phone-number');
        
        if (requesterNameSpan && requesterData.name) requesterNameSpan.textContent = requesterData.name;
        if (phoneNumberSpan) phoneNumberSpan.textContent = callingNumber;
...
function extractCallingNumber() {
  ...
  const iframe = document.querySelector('iframe[allow*="camera"]') as HTMLIFrameElement;
...
// Execute the main function when sidepanel loads
main();